...
async function migrateAllData() {
  console.log('🚀 Migrating ALL data from SQLite to PostgreSQL...\n')

  const db = sqlite3.default('./prisma/dev.db')

  try {
    // 1. Migrate Users (already done, but doing it again to be safe)
    ...
    for (const person of people) {
      try {
        await postgres.personReference.upsert({
          where: { id: person.id },
          update: {
    ...
    for (const tag of tags) {
      try {
        await postgres.entryTag.upsert({
          where: { id: tag.id },
          update: {
            entryId: tag.entryId,
            userId: tag.userId,
    ...
    for (const fr of friendRequests) {
      try {
        await postgres.friendRequest.upsert({
          where: { id: fr.id },
          update: {
            ...
            updatedAt: new Date(fr.updatedAt),
          },
          create: {
            id: fr.id,
            senderId: fr.senderId,
    ...
    console.log('Your full history has been restored to PostgreSQL.')
  } catch (error: any) {
